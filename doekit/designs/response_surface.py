...
from ..factors import Factor, ContinuousFactor
from ..model import Model
from .base import Design


def _resolve_factors(spec, n_hint: Optional[int] = None):
    """Normalize the factor specification for RSM designs.
...
def _decode(coded: np.ndarray, factors: Optional[Sequence[Factor]]) -> np.ndarray:
    """Decode a coded matrix column-by-column to natural units (identity if no factors)."""
    if factors is None:
        return coded
