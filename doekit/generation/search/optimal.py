...
from ...domain.criteria.linalg import leverage as _leverage, inv_info as _inv
...
def optimal_design(candidates: Design, n_runs: int, model: Optional[Model] = None,
    ...
    **kl_kwargs
    ...
    crit_key = criterion.strip().upper()
    crit_fn = _crit.get_criterion(crit_key)
    ctx = CriterionContext(moment_matrix=X_full if crit_key == "I" else None,
                           tolerance=tolerance)

    if algorithm is None:
        algorithm = "kl" if crit_key == "D" else "fedorov"

    def eval_score(idx):
        return score_criterion(crit_fn, X_full[idx], ctx)
    ...
    for _ in range(max(1, n_starts)):
        ...
        if algorithm == "kl":
            ...
            moments = X_full if crit_key == "I" else None
    ...
    report_ctx = CriterionContext(moment_matrix=X_full)
    crits = _crit.all_criteria(X_best, report_ctx)
    meta = {
        "kind": "OptimalDesign",
        "criterion": crit_key,
        "algorithm": algorithm,
    ...
    if report is not None:
        ...
        path = maybe_report(result, report=report, model=model, seed=seed)
        if path is not None:
