...
same quality language as :mod:`doekit.evaluate` (D/A/G-efficiency, SPV, power).
"""
...
from .evaluate import efficiencies, power_analysis
from .factors import ContinuousFactor, DiscreteFactor, CategoricalFactor
...
def _default_candidates(design: Design, model: Model, n_cand: int,
    ...
    cand.factors = facs
    cand.model = model
    # Keep only factor columns present in the model / design
    keep = [c for c in names if c in cand.matrix.columns]
    if keep:
        cand.matrix = cand.matrix.loc[:, keep].copy()
    return cand
...
def _score(X: np.ndarray, criterion: str, X_region: Optional[np.ndarray]) -> float:
    ...
    if c == "I":
        return _crit.i_criterion(X, X_region if X_region is not None else X)
    return _crit.get_criterion(c)(X)
...
def compare_designs(a: Design, b: Design, model: Optional[Model] = None,
    ...
    metric delta table (same language as :func:`~doekit.evaluate.evaluate`).
...
def propose_next_runs(design: Design, response=None, n_add: int = 4,
    ...
    if response is not None:
        from .analysis import fit_linear_model  # noqa: PLC0415
